...
class WorkingOrders
{
    ...
    public function getWorkingOrders(){
       ...
       // $shopStatment = "and shop_id = 2";
        ...
        $query = "SELECT `orders`.`id` order_id,`orders`.`created_at` order_date,`orders`.`order_status`,`orders`.total,`orders`.delivery_charge,`orders`.qty,`user`.`username` delivery_user , `shops`.id shop_id,`shops`.subscribed_in_delivery subscribed_in_delivery, `shops`.`name` shop_name, `customers`.id customer_id,`customers`.full_name customer_full_name,
                                                `customer_addresses`.`id` customer_address_id, concat('[',`cities`.`name`,' - ',`areas`.`name`,' - ',`customer_addresses`.`street`,' - ',
