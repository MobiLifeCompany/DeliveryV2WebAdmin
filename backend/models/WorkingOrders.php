...
class WorkingOrders
{
    ...
    public function getWorkingOrders(){
        ...
        $query = "SELECT `orders`.`shop_id` shop_id, `orders`.`id` order_id,`orders`.`note` order_note,`orders`.ready_time ready_time,`orders`.`created_at` order_date,`orders`.`order_status`,`orders`.total,`orders`.delivery_charge,`orders`.qty,`user`.`username` delivery_user , `shops`.id shop_id,`shops`.subscribed_in_delivery subscribed_in_delivery, `shops`.`name` shop_name,`shops`.`ar_name` ar_shop_name, `customers`.id customer_id,`customers`.full_name customer_full_name,
